...
def get_elements_3s(driver, element_meta_data, time_sum=3):
    """
    :param time_sum:
    :param element_meta_data:
    :type element_meta_data:
    ...
    try:
        if by == 'IOS_UIAUTOMATION':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_ios_uiautomation(value))
        if by == 'ANDROID_UIAUTOMATOR':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_android_uiautomator(value))
        if by == 'ACCESSIBILITY_ID':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_accessibility_id(value))
        if by == 'RESOURCE_ID':
            return  WebDriverWait(driver, time_sum).until(lambda x: x.find_element_by_resource_id(value))
        if by == 'ID':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_id(value))
        if by == 'XPATH':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_xpath(value))
        if by == 'LINK_TEXT':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_link_text(value))
        if by == 'PARTIAL_LINK_TEXT':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_partial_link_text(value))
        if by == 'NAME':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_name(value))
        if by == 'TAG_NAME':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_tag_name(value))
        if by == 'CLASS_NAME':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_class_name(value))
        if by == 'CSS_SELECTOR':
            return WebDriverWait(driver, time_sum).until(lambda x: x.find_elements_by_css_selector(value))
        else:
            raise
    except Exception as e:
        print(e)
        print('没有找到(' + by + ":" + value + ')元素')
        # run_info_log(str(traceback.format_exc()), GlobalVarClass.get_log_file())
        # run_info_log(e, GlobalVarClass.get_log_file())
        raise
...
def get_element_3s(driver, element_meta_data, time_sum=3):
    """
...
def is_element_present_3s(driver, locator, time=3):
    """
    ...
    try:
        ...
        print(e)
        try:
            get_elements_3s(driver, locator, time)
            return True
        except Exception as e:
            print(e)
            return False
